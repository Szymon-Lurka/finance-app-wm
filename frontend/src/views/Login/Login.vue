      ...
      handleSubmit(async ({ email, password }) => {
        try {
          ...
          if (e.response.status === 400 || e.response.status === 404) {
            dispatchErrorToast({
              title: lang.auth.titles.login,
              details: lang.auth.error.details.exists,
            });
          } else {
            dispatchErrorToast({
              title: lang.auth.titles.login,
              details: lang.auth.error.details.login,
            });
          }
        }
      })();
    ...
    <template #content>
      <form @submit.prevent="onSubmit()" class="form">
        <div class="form-group">
          ...
          <Button severity="info" @click="goToRegister"> Zarejestruj się </Button>
          <Button severity="info" @click="goToForgotPassword"> Przypomnij hasło </Button>
        </div>
      </form>
    </template>
  </Card>
