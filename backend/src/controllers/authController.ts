...
} from "../utils/tokens/createTokens";
...
import {
    ...
    nodemailerSetup,
} from "../utils/email/nodemailerSetup";
...
const forgotPassword = async (
    req: CustomRequest<{}, ForgotPasswordBody>,
    res: Response,
    next: NextFunction
) => {
    ...
    if (!email) {
        return next(
            new BodyFieldsValidationError("Forgot password email", ["email"])
        );
    }
    ...
    if (!user) {
        return next(
            new NotFoundError("Not found", "User with provided email doesn't exists.")
        );
    }
    ...
    const data = {
        link: `${process.env.CLIENT_URL}/#/auth/forgot-password/${resetPasswordToken}`,
        name: user.firstName,
    };
...
const resetPassword = async (
    req: CustomRequest<{}, ResetPasswordBody>,
    res: Response,
    next: NextFunction
) => {
    ...
    if (newPassword !== repeatPassword) {
        return next(new ValidationError('Password are not the same', 'Password must be the same'))
    }

    if (!token) {
        return next(
            new BodyFieldsValidationError("Wrong reset password token", ["token"])
        );
    }

    if (!newPassword || !PASSWORD_VALIDATION_REGEX.test(newPassword)) {
        return next(
            new ValidationError("Wrong password reset password", errors.auth.password)
        );
    }
    ...
    if (user.resetPasswordToken !== token) {
        return next(
            new ValidationError(
                "Wrong token reset password",
                "Token for password reset is incorrect."
            )
        );
    }
...
const register = async (
    ...
    if (invalidFields.length > 0) {
        return next(
            new BodyFieldsValidationError("Register wrong data", invalidFields)
        );
    }
...
const refreshToken = async (
    req: CustomRequest<{}, RefreshTokenBody>,
    res: Response,
    next: NextFunction
) => {
    ...
    if (!refreshToken) {
        return next(
            new UnauthorizedError("RefreshToken", errors.auth.refreshToken)
        );
    }
